...
当前存在的问题：
1. 做成无状态工具，混杂了工具逻辑与个人配置。需要解耦
1. 阶段职责错位。 install阶段直接执行adapt，混淆了"部署"与"适配"的边界。install应只负责技能本体落盘，enable阶段再根据目标agent执行环境适配。
2. 配置空间耦合。 关联文件统一堆放在.agent目录，缺乏agent级隔离。应按目标agent拆分为独立目录（opencode/、codex/、claude-code/），由用户显式选择适配对象。
3. 适配逻辑未技能化。 adapt本质是动态执行逻辑而非静态资源，应抽象为独立skill，在enable阶段通过脚本化方式按需调用。


- install 只执行下载
- enable/link 是否需要做成skill
- adapt 需要做成skill
