# hk-skills

## prompt-optimizer
- [一泽Eze](https://chatmemo.feishu.cn/wiki/NtmCwdODziaHsikKFJGclN64n4e)
- [skills](https://github.com/chujianyun/skills/tree/main/skills)
- MIPROv2 + DSPY 实现 APO（自动提示优化）
- https://my.feishu.cn/wiki/MrgbwlxSFitOMbkii9EcWUegnpb?fromScene=spaceOverview

## skill 学习
- [anthropics/skills](https://github.com/anthropics/skills)
- [baoyu-skills](https://github.com/JimLiu/baoyu-skills/tree/main/skills)
- [ai-partner-chat](https://github.com/eze-is/ai-partner-chat)
- [skillsmp](https://skillsmp.com/zh)
- [skills](https://skills.sh/)
- [企业级 Skills](https://github.com/mrgoonie/claudekit-skills)：https://github.com/ComposioHQ/awesome-claude-skills
- [Copilot 官方 Skills](https://github.com/github/awesome-copilot/tree/main/skills)