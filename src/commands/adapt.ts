...
export async function adaptCommand(root: string, name: string): Promise<void> {
  ...
  if (entry.source_id) {
    ...
    if (sourceEntry?.type === "local") {
      sourcePath = path.resolve(root, sourceEntry.local_path);
      sourceType = "local";
    } else if (sourceEntry?.type === "remote") {
      sourcePath = path.join(getWarehousePath(root, "remote"), entry.source_id);
      if (entry.subpath) {
        sourcePath = path.join(sourcePath, entry.subpath);
      }
      sourceType = "remote";
    }
  }
