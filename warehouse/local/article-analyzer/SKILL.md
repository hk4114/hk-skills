...
Route before analyzing. This skill is a routing system, not one giant prompt. Select the smallest module chain that answers the user task.
...
2. Infer user goal.
3. Build the smallest valid module chain.
4. Load only the matching reference prompt files.
5. Produce the requested output mode with evidence labels where claims need traceability.

If the source type is ambiguous, default to `article` and run `deep_analysis`.

### Source Types

| Source family | Signals | Default module |
| --- | --- | --- |
| `paper` | paper, 论文, arXiv, DOI, abstract, method, experiments, SOTA, contribution, benchmark | `paper_scan` |
| `article` | article, essay, blog, report, commentary, long-form argument | `deep_analysis` |
| `technical_article` | technical blog, engineering article, architecture explanation, method comparison | `deep_analysis` |
| `business_commentary` | market, company, strategy, industry, product, operation, investment commentary | `deep_analysis` |
| `raw_thought` | rough notes, verbose draft, conversation excerpt, loose argument | minimal `deep_analysis -> thought_refine` |

### Goal Routing

| User intent | Signals | Module chain |
| --- | --- | --- |
| paper quick read | 论文速读, 快速读懂论文, paper scan, arXiv | `paper_scan` |
| deep analysis | 深度分析, 分析这篇文章, 拆解观点, 看看这篇 | `deep_analysis` |
| thought refinement | 思想精炼, 提炼主线, 压缩成结论, 高密度表达, 结论先行 | `deep_analysis -> thought_refine` |
| cognitive upgrade | 提维, 升维, 认知升级, 底层模型, 新框架, 重构模型 | `deep_analysis -> cognitive_upgrade` |
| paper deep analysis | 分析这篇论文, 论文解构, 方法论文分析 | `paper_scan -> deep_analysis` |

When multiple goals are present, preserve this order:

`paper_scan -> deep_analysis -> thought_refine -> cognitive_upgrade`

Only include later modules when the user explicitly requests them or the goal clearly implies them.
...
| --- | --- | --- |
| `analysis_report` | default deep analysis | structured Markdown report |
| `paper_brief` | paper quick read | high-density paper deconstruction |
| `refined_thesis` | thought refinement | conclusion-first thesis and supports |
| `upgrade_model` | cognitive upgrade | evidence-layered synthesis model |
| `combined` | user asks for multiple layers | one report per selected module, ordered by selected module chain, followed by a final synthesis |

Default output should be compact but not shallow. Prefer concrete claims, evidence, and boundaries over generic explanation.

For `combined`, move step by step: each selected module generates its own report, then end with `最终总结`. Do not generate reports for skipped modules. Internal lightweight `deep_analysis` used only to support `thought_refine` or `cognitive_upgrade` does not get a standalone `analysis_report`.

## Failure Guards

- Do not run all modules by default.
